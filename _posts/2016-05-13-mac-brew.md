...
+ 开启虚拟主机后原来`/Library/WebServer/Documents`的项目就不能访问了，所以要添加下面的配置，把默认位置配置为虚拟主机就可以了
...
但发现无法访问`http://localhost:9999/` 试了很多方法都不行 最后发现把`系统的文件共享`打开就行了  即使再关闭共享 依旧可以 不知道是不是mac的bug
